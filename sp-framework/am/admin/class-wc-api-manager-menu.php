...
class API_Manager_Massimo_theme_MENU {

	private $api_manager_theme_massimo_key;

	// Load admin menu
	public function __construct() {

		$this->api_manager_theme_massimo_key = AMET()->api_manager_theme_massimo_key;
	...
	public function validate_options( $input ) {
		...
		if ( $_REQUEST['option_page'] != AMET()->ame_deactivate_checkbox ) {
			...
			if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key  ) {
				...
				$activate_results = json_decode( $this->api_manager_theme_massimo_key->activate( $args ), true );
	...
	public function replace_license_key( $current_api_key ) {
		...
		$reset = $this->api_manager_theme_massimo_key->deactivate( $args ); // reset license key activation
	...
	public function wc_am_license_key_deactivation( $input ) {
		...
		// $activate_results = json_decode( $this->api_manager_theme_massimo_key->status( $args ), true );
		...
		if ( $options == 'on' && $activation_status == 'Activated' && AMET()->ame_options[AMET()->ame_api_key] != '' && AMET()->ame_options[AMET()->ame_activation_email] != '' ) {
			...
			$activate_results = json_decode( $this->api_manager_theme_massimo_key->deactivate( $args ), true );
...
new API_Manager_Massimo_theme_MENU();
