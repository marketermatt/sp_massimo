...
if ( get_option( 'api_manager_theme_massimo_activated' ) != 'Activated' ) {
	add_action( 'admin_notices', 'API_Manager_theme_Massimo::am_theme_massimo_inactive_notice' );
}


class API_Manager_theme_Massimo {
	 ...
	 */
	public $api_manager_theme_massimo_version_name = 'api_manager_theme_massimo_version';
	 ...
	 */
	public $text_domain = 'api-manager-theme-massimo';
	...
	public $ame_update_check = 'am_massimo_theme_update_check';

	public $api_manager_theme_massimo_key;
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'api-manager-massimo' ), '1.2' );
	}
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			 ...
			 */
			$this->ame_data_key 				= 'api_manager_theme_massimo';
			$this->ame_api_key 					= 'api_key';
			$this->ame_activation_email 		= 'activation_email';
			$this->ame_product_id_key 			= 'api_manager_theme_massimo_product_id';
			$this->ame_instance_key 			= 'api_manager_theme_massimo_instance';
			$this->ame_deactivate_checkbox_key 	= 'api_manager_theme_massimo_deactivate_checkbox';
			$this->ame_activated_key 			= 'api_manager_theme_massimo_activated';
			 ...
			 * so only the host portion of the URL can be sent. For massimo the host portion might be
			 * www.massimo.com or massimo.com. http://www.massimo.com includes the scheme http,
			 * and the host www.massimo.com.
	 ...
	 * Update Check Class.
	 *
	 * @return API_Manager_Massimo_theme_Update_API_Check
	 */
	public function update_check( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra = '' ) {
		...
		return API_Manager_Massimo_theme_Update_API_Check::instance( $upgrade_url, $plugin_name, $product_id, $api_key, $activation_email, $renew_license_url, $instance, $domain, $software_version, $plugin_or_theme, $text_domain, $extra );
	}
	...
	public function activation() {
		...
		if ( get_option( $this->ame_data_key ) === false || get_option( $this->ame_instance_key ) === false ) {
			...
			$api_manager_theme_massimo_password_management = new API_Manager_Massimo_theme_Password_Management();

			// Generate a unique installation $instance id
			$instance = $api_manager_theme_massimo_password_management->generate_password( 12, false );
			...
			$curr_ver = get_option( $this->api_manager_theme_massimo_version_name );

			// checks if the current plugin version is lower than the version being installed
			if ( version_compare( $this->version, $curr_ver, '>' ) ) {
				// update the version
				update_option( $this->api_manager_theme_massimo_version_name, $this->version );
			}
	...
	public function license_key_deactivation() {
		...
		if ( $activation_status == 'Activated' && $api_key != '' && $api_email != '' ) {
			$this->api_manager_theme_massimo_key->deactivate( $args ); // reset license key activation
		}
	}
     ...
     */
	public static function am_theme_massimo_inactive_notice() { ?>
		<?php if ( ! current_user_can( 'manage_options' ) ) return; ?>
		<?php if ( isset( $_GET['page'] ) && 'api_manager_theme_massimo_dashboard' == $_GET['page'] ) return; ?>
		<div id="message" class="error">
			<p><?php printf( __( 'The API Manager Theme Massimo API License Key has not been activated, so the theme is inactive! %sClick here%s to activate the license key and the theme.', 'api-manager-massimo' ), '<a href="' . esc_url( admin_url( 'index.php?page=api_manager_theme_massimo_dashboard' ) ) . '">', '</a>' ); ?></p>
		</div>
		<?php
	}
	...
	public function check_external_blocking() {
		...
		if( defined( 'WP_HTTP_BLOCK_EXTERNAL' ) && WP_HTTP_BLOCK_EXTERNAL === true ) {
			...
			if( ! defined( 'WP_ACCESSIBLE_HOSTS' ) || stristr( WP_ACCESSIBLE_HOSTS, $host ) === false ) {
				?>
				<div class="error">
					<p><?php printf( __( '<b>Warning!</b> You\'re blocking external requests which means you won\'t be able to get %s updates. Please add %s to %s.', 'api-manager-massimo' ), $this->ame_product_id, '<strong>' . $host . '</strong>', '<code>WP_ACCESSIBLE_HOSTS</code>'); ?></p>
				</div>
				<?php
			}
...
function AMET() {
    return API_Manager_theme_Massimo::instance();
}
