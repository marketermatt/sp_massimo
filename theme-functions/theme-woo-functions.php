...
function sp_display_wishlist_compare_icons() {
	...
	if ( sp_woo_exists() && ( sp_get_option( 'show_wishlist_icon', 'is', 'on' ) || sp_get_option( 'show_compare_icon', 'is', 'on' ) ) ) {
		...
		$output .= '<div class="wishlist-compare-tab">' . PHP_EOL;
		...
		if ( sp_get_option( 'show_compare_icon', 'is', 'on' ) )
			$output .= '<a href="' . get_permalink( sp_compare_page_id() ) . '" title="' . esc_attr__( 'Compare Products', 'sp-theme') . '" class="get-compare sp-tooltip" data-toggle="tooltip" data-placement="top"><i class="icon-cog" aria-hidden="true"><span class="compare-item-count">' . sp_woo_get_compare_item_count() . '</span></i></a>' . PHP_EOL;
